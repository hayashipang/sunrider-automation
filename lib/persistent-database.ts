...
const initializeData = () => {
  ...
  // 嘗試從文件加載數據
  memorySolutions = loadSolutionsFromFile()
  memoryImages = loadImagesFromFile()
  
  // 如果沒有文件數據，則使用默認數據
  if (memorySolutions.length === 0) {
    memorySolutions = [
      {
        id: 'solution-1',
        title: '製造業自動化',
    ...
    // 保存默認數據到文件
    saveSolutionsToFile(memorySolutions)
  }
  
  if (memoryImages.length === 0) {
    memoryImages = []
    saveImagesToFile(memoryImages)
  }
